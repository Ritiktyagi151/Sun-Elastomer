...
export function ProductCategoryPage({ categorySlug }: ProductCategoryPageProps) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.error("Failed to fetch products/categories for page:", err);
        setLocalCategories(productCategories);
      } finally {
        setLoading(false);
      }
  ...
  const category = localCategories.find((item) => {
    const itemCat = item.category || "";
    return itemCat.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") === categorySlug;
  });
    ...
    return (
      <main className="flex min-h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-semibold text-muted">Loading category…</p>
        </div>
      </main>
    );
  }
  ...
  const activeIcon = category.iconName || category.icon;
  const IconComponent = typeof activeIcon === "string"
    ? iconMap[activeIcon] || Beaker
    : activeIcon || Beaker;

  const categoryProducts = localProducts.filter((product) => {
    if (!product || !product.category || !category.category) return false;
    const slug1 = product.category.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    const slug2 = category.category.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    return slug1 === slug2;
  });
