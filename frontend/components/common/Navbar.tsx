...
export function BrandLogo({ scrolled = false }: { scrolled?: boolean }) {
  return (
    <Link
      href="/"
      className={`group relative z-10 inline-flex items-center justify-center transition-all duration-300 ${scrolled ? "top-0 h-16 w-64" : "-top-5 h-24 w-80"
        }`}
...
export function Navbar() {
  ...
  return (
    <>
      ...
      <motion.div
        className={`fixed inset-x-0 top-0 z-50 hidden lg:block transition-all duration-300 ${scrolled ? "-translate-y-full opacity-0" : "translate-y-0 opacity-100"
          }`}
      ...
      <motion.header
        className={`fixed inset-x-0 right-0 left-auto top-0 z-50 w-full transition-all duration-300 ${scrolled ? "top-0 shadow-2xl shadow-crimson/10" : "top-0 lg:top-8"
          }`}
      ...
      >
        ...
        >
          ...
          >
            {navLinks.map((link) =>
              link.href === "/categories" ? (
                <ProductDropdown key={link.href} scrolled={scrolled} categories={categories} />
              ) : (
                <motion.div
                  key={link.href}
                ...
                >
                  <Link
                    href={link.href}
                    className={`relative text-sm font-semibold transition-all duration-300 after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 after:bg-crimson after:transition-all after:duration-300 hover:after:w-full ${scrolled ? "text-ink/80 hover:text-crimson" : "text-white hover:text-white"
                      }`}
                  >
                    {link.label}
                  </Link>
          ...
          <div className="flex items-center justify-end gap-3">
            {/* Search Icon Button */}
            <motion.button
              className={`hidden lg:grid h-10 w-10 place-items-center rounded-full border transition-all duration-300 hover:shadow-md ${scrolled
            ...
            >
              <button
                type="button"
                className={`relative inline-flex items-center gap-2 overflow-hidden rounded-full px-5 py-2.5 text-sm font-semibold text-white transition-all duration-300 hover:scale-105 active:scale-95 shadow-md ${scrolled
            ...
            {/* Mobile hamburger */}
            <button
              className={`grid h-11 w-11 place-items-center rounded-full border transition-all duration-300 lg:hidden ${scrolled
                  ...
                  <div className="ml-3 space-y-1 border-l border-crimson/10 pl-3">
                    {categories.map((category) => {
                      ...
                      return (
                        ...
                        >
                          <IconComponent size={16} className="shrink-0 text-golden" />
                          {category.title}
                        </Link>
                      );
                    })}
...
function ProductDropdown({ scrolled, categories }: { scrolled: boolean; categories: any[] }) {
  ...
  return (
    ...
    >
      <button
        ...
        className={`flex items-center gap-1.5 text-sm font-medium transition-all duration-300 ${scrolled ? "text-ink/80 hover:text-crimson" : "text-white/90 hover:text-white"
          }`}
      >
        Products
        <ChevronDown
      ...
      <div
        className={`absolute left-1/2 top-full z-50 w-[26rem] -translate-x-1/2 rounded-2xl border border-crimson/5 bg-white/95 p-4 shadow-2xl shadow-crimson/20 backdrop-blur-xl transition-all duration-200 ${dropdownOpen ? "visible mt-3 opacity-100" : "invisible mt-5 opacity-0"
          }`}
      >
