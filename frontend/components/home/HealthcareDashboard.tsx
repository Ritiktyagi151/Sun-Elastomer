...
export default function SunElastomersDashboard() {
  ...
  const [counts, setCounts] = useState({ products: 15, categories: 6 });
    ...
    async function loadStats() {
      try {
        ...
        if (prodsRes.ok && catsRes.ok) {
          ...
          setCounts({ products: prods.length, categories: cats.length });
        }
      } catch (err) {
  ...
  const menuItems: MenuItem[] = [
    ...
    { id: "support",   label: "Focused support",    icon: Headphones },
  ...
  const stats: StatItem[] = [
    { label: "Products",     value: counts.products, suffix: "", icon: Building2 },
    { label: "Categories",   value: counts.categories,    suffix: "",  icon: LayoutGrid },
    { label: "Dosage Forms", value: 5,   suffix: "+", icon: Droplet   },
