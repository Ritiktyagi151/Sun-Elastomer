...
export const company = {
  get name() {
    if (typeof window !== "undefined") {
      ...
      if (stored) return JSON.parse(stored).name || "Sun Elastomers Private Limited";
    }
    return "Sun Elastomers Private Limited";
  },
  get shortName() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).shortName || "Sun Elastomers";
    }
    return "Sun Elastomers";
  },
  get gstin() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).gstin || "09AACCC9768N1ZS";
    }
    return "09AACCC9768N1ZS";
  },
  get registrationDate() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).registrationDate || "30/10/2018";
    }
    return "30/10/2018";
  },
  get registrationType() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).registrationType || "Regular";
    }
    return "Regular";
  },
  get constitution() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).constitution || "Private Limited Company";
    }
  ...
  get address() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).address || "Site-4, 62/2/2, Industrial Area Sahibabad, Ghaziabad, Uttar Pradesh - 201010";
    }
    return "Site-4, 62/2/2, Industrial Area Sahibabad, Ghaziabad, Uttar Pradesh - 201010";
  },
  get contactEmail() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).contactEmail || "info@sunelastomerspharma.com";
    }
    return "info@sunelastomerspharma.com";
  },
  get contactPhone() {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("sun_company_info");
      if (stored) return JSON.parse(stored).contactPhone || "+91 99677 77537";
    }
