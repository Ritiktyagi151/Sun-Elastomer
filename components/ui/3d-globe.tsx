...
function Marker({ marker, radius, defaultSize, onClick, onHover }: MarkerProps) {
  ...
  useFrame(() => {
    ...
    const nextVisible = markerDirection.dot(cameraDirection) > 0.1;
  ...
  const markerSize = `${Math.max(8, (marker.size ?? defaultSize) * 140)}px`;
  ...
  return (
    <group visible={isVisible}>
      ...
      <mesh position={surfacePosition} quaternion={lineQuaternion}>
        ...
        <meshBasicMaterial color={hovered ? "#1a1a1a" : "#b91c1c"} />
      </mesh>
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <button
            type="button"
            className={cn(
              "overflow-hidden rounded-full border border-white/70 bg-white shadow-lg transition-transform duration-200",
              hovered && "scale-125 shadow-xl ring-1 ring-white/70"
            )}
            style={{
              backgroundImage: `url(${marker.src})`,
              backgroundPosition: "center",
              backgroundSize: "cover",
              height: markerSize,
              width: markerSize,
            }}
            aria-label={marker.label ? `View ${marker.label}` : "View global marker"}
  ...
  return (
    <group rotation={[config.initialRotation.x, config.initialRotation.y, 0]}>
      ...
      {markers.map((marker, index) => (
        <Marker
          key={`${marker.label ?? "marker"}-${index}-${marker.lat}-${marker.lng}`}
          marker={marker}
...
function LoadingFallback() {
  return (
    <Html center>
      <span className="text-sm font-semibold text-neutral-500">Loading global network...</span>
    </Html>
  );
}
